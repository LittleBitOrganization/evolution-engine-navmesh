...
namespace UnityEditor.AI
{
    ...
    class NavMeshSurfaceEditor : Editor
    {
        ...
        void OnEnable()
        {
            ...
            m_roads = serializedObject.FindProperty("m_roads");
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(Application.isPlaying || m_AgentTypeID.intValue == -1))
            {
                ...
                {
                    (target as NavMeshSurface).FindAllRoads();
                    (target as NavMeshSurface).RoadsEnable();
                ...
                {
                    foreach (var navMesh in targets)
                    {
                        (navMesh as NavMeshSurface).FindAllRoads();
                        (navMesh as NavMeshSurface).RoadsEnable();
                    }
            ...
            if(GUILayout.Button("DisableRoads"))
            {
                (target as NavMeshSurface).RoadsDisable();
            }
            EditorGUILayout.PropertyField(m_roads);
        }
